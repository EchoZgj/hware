 package com.hware.tools;


 import com.hware.util.ImageCompress;
 import com.hware.util.PageBean;
 import com.hware.util.StringUtils;
 ...
 public class CommUtil
 {
    ...
    * 上传文件，可以保存原文件名称
    * @param request
    * @param fileGetName 获取上传文件的元素的名称
    * @param saveFilePath 文件的存储路径
    * @param saveFileName 文件的储存名称
    * @param userFileSelfName 是否使用文件的原名称
    * @param extendes 扩展名集合
    * @return
    * @throws IOException
    */
   public static File saveFileToServer(HttpServletRequest request, String fileGetName, String saveFilePath, String saveFileName, boolean userFileSelfName, String[] extendes)
           throws IOException
   {
     MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest)request;
     CommonsMultipartFile file = (CommonsMultipartFile)multipartRequest
     ...
     if ((file != null) && (!file.isEmpty()))
     {
       ...
       if(userFileSelfName){
         ...
         saveFileName = originalFileName.substring(0, originalFileName.lastIndexOf(".")) + "r_" + random.nextInt(10) + "" + random.nextInt(10) + "" + random.nextInt(10) + "";
       }else if((saveFileName == null) || (saveFileName.trim().equals(""))){
         saveFileName = UUID.randomUUID().toString() + "." + extend;
       }
       if (saveFileName.lastIndexOf(".") < 0) {
         saveFileName = saveFileName + "." + extend;
     ...
     return new File(saveFilePath + File.separator + saveFileName);
   }
   ...
   public static void savePageBeanModelAndView(String url, String staticURL, String params, PageBean pageBean, ModelAndView mv){

    if(pageBean!=null) {
      mv.addObject("objs", pageBean.getList());
      mv.addObject("totalPage", pageBean.getTotalPage());
      mv.addObject("pageSize", pageBean.getPageSize());
      mv.addObject("rows", pageBean.getAllRow());
      mv.addObject("currentPage", pageBean.getCurrentPage());
      mv.addObject("gotoPageHTML", showPageHtml(url, params,
              pageBean.getCurrentPage(), pageBean.getTotalPage()));
      mv.addObject(
              "gotoPageFormHTML",
              showPageFormHtml(pageBean.getCurrentPage(), pageBean.getTotalPage()));
      mv.addObject(
              "gotoPageStaticHTML",
